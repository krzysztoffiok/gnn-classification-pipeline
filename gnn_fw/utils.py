...
def preprocess_uj_data(brain_parcellation, feature_set, threshold, printout=False):
    """
    TODO: it is possible to create a common function for preprocessing of all MRI data sets. Now, HCP data and UJ data
    have two separate functions which share a lot of code.

    A function fo preprocess MRI data from the Jagiellonian University 
    :param brain_parcellation: int = 200
    ...
    filenames = ["fmri_bin.npy",
                 "fmri_corr.npy",
                 "fmri_ts.npy"]

    dir_name = f"{config.directories['source data']}/uj_{brain_parcellation}"
    filenames = [f"{dir_name}/{x}" for x in filenames]
    test_run_name = f"uj_{brain_parcellation}_{threshold}_{features}"
    load = False
    ...
    # load files
    bin_ = np.load(filenames[0])
    corr_ = np.load(filenames[1])
    ts_ = np.load(filenames[2])

    time_series_data_points = ts_.shape[1]

    # list of source nodes
    index_of_source_nodes = [x for x in range(bin_[0].shape[0])]
    ...
    for patient in range(corr_.shape[0]):
        ...
        if printout:
            print(f"Computation for a single patient took:"
                  f" {patient_end - patient_start}. {bin_.shape[0] - patient} patients to compute.")
...
def create_uj_dataset(correlation_matrix, time_series, index_of_source_nodes, index_of_target_nodes_dict, threshold,
                      node_feature_dict, brain_parcellation, test_run_name):
    """
    TODO: it is possible to create a common function for creating all MRI data sets. Now, HCP data and UJ data
    have two separate functions which share a lot of code.
    ...
    if os.path.isfile(dataset_filepath):
        ...
        y_list = [0 for _ in range(61)]
        y_list.extend([1 for _ in range(122-61)])
        return torch.load(dataset_filepath), y_list
        ...
        for patient in range(correlation_matrix.shape[0]):  # patient is a number of patient
            ...
            for k in range(time_series[0].shape[1]):
                x_list.append(node_feature_dict[f"{patient}.{k}"])

            x = torch.tensor(x_list, dtype=torch.float)

            # define target y according to original data
            if patient <= 61:
                y = 0
            else:
                y = 1
            ...
            data = Data(x=x, edge_index=edge_index, edge_attr=edge_attributes, y=y, num_nodes=brain_parcellation)
            dataset.append(data)

        torch.save(dataset, dataset_filepath)
        return dataset, y_list
...
def create_instance_graph(data, spanning_tree="full"):
    """
    ...
    :param data:
    :param spanning_tree: string, a name of the spanning tree option (full-all edges are retained, other names result
    ...
    for edge in range(number_of_edges):
        ...
        # else:
            # print("Graphs with multiple edge attrs are not supported. Using the first edge attribute only.")
        # else:
        #     edge_weight = np.reshape(data.edge_attr[edge].numpy(), (1, data.edge_attr[edge].shape[0]))[0][0]
        # edge_weight_dict = {str(num): v for num, v in enumerate(edge_weight)}
        # print(edge_weight)
    ...
    if has_edge_weights:
        if spanning_tree.find("st") != -1:
            if spanning_tree == "maxst":
                st_graph = nx.maximum_spanning_tree(graph)
            elif spanning_tree == "minst":
                st_graph = nx.minimum_spanning_tree(graph)
            elif spanning_tree == "mixedst":
...
def add_networkx_node_features(dataset, feature="degree"):
    ...
    for num, data in enumerate(dataset):
        ...
        graph = create_instance_graph(data, spanning_tree="no")
        x = data.x
        y = data.y
...
def create_mst_data(data, spanning_tree):
    """
    ...
    mst_graph = create_instance_graph(data, spanning_tree=spanning_tree)
...
def visualize_graph(data_instance_number, dataset, dataset_name, spanning_tree):
    """
    ...
    graph_list = create_instance_graph(dataset[data_instance_number])
    if spanning_tree:
        graph_list.append(create_instance_graph(dataset[data_instance_number], spanning_tree))
...
def compute_node_embeddings(node_embedding_parameters, dataset, test_run_name, printout=True,
    ...
    if os.path.isfile(dataset_filepath):
        ...
        for patient_data in range(len(dataset)):
            ...
            graph = create_instance_graph(data)
...
def preprocess_hcp_data(task, dataset_name, brain_parcellation, feature_set, threshold, printout=False):
    """
    TODO: it is possible to create a common function for creating all MRI data sets. Now, HCP data and UJ data
    have two separate functions which share a lot of code.
    A function which creates the dataset (a list of pytorch.geometric Data objects) from preprocessed UJ data.
    ...
    # bin_ = np.load(filenames[0])
    corr_ = np.round(np.load(filenames[1]).astype(np.float16), 3)
    ts_ = np.round(np.load(filenames[2]).astype(np.float16), 3)
...
def create_hcp_dataset(correlation_matrix, index_of_source_nodes, index_of_target_nodes_dict, threshold,
                       node_feature_dict, brain_parcellation, test_run_name):
    """
    TODO: it is possible to create a common function for creating all MRI data sets. Now, HCP data and UJ data
    have two separate functions which share a lot of code.
    ...
    if os.path.isfile(dataset_filepath):
        ...
        for patient in range(correlation_matrix.shape[0]):  # patient is a number of patient
            ...
            # define dummy target = 0
            y = 0

            # create data objects and append them in a list
            data = Data(x=x, edge_index=edge_index, edge_attr=torch.tensor(edge_attributes, dtype=torch.float32),
...
def skf_splitter(nfolds, y_list):
    """
...
def load_dataset(config):
    ...
    """
    # loading the dataset
    if "uj" in config.selected_dataset:
        # load data, format for torch geometric data and extract time series features
        index_of_source_nodes, index_of_target_nodes_dict, node_feature_dict, test_run_name, correlation_matrix,\
            time_series = gfw.utils.preprocess_uj_data(brain_parcellation=config.brain_parcellation,
                                                       feature_set=config.feature_set,
                                                       threshold=config.threshold, printout=config.printout)
        # create the uj_dataset
        dataset, y_list = gfw.utils.create_uj_dataset(correlation_matrix, time_series, index_of_source_nodes,
                                                      index_of_target_nodes_dict, config.threshold, node_feature_dict,
                                                      config.brain_parcellation, test_run_name)

        test_run_name_for_whole_dataset = test_run_name

    elif "mutag" in config.selected_dataset:
    ...
    elif config.selected_dataset.find("hcp") != -1:
        dataset = list()
        for task in config.tasks_list:
            index_of_source_nodes, index_of_target_nodes_dict, node_feature_dict, test_run_name, correlation_matrix, \
             = gfw.utils.preprocess_hcp_data(task, config.selected_dataset, config.brain_parcellation,
                                             config.feature_set, config.threshold,
                                             printout=config.printout)
            # create the hcp_dataset
            local_dataset = gfw.utils.create_hcp_dataset(correlation_matrix, index_of_source_nodes,
                                                         index_of_target_nodes_dict, config.threshold,
                                                         node_feature_dict, config.brain_parcellation, test_run_name)
